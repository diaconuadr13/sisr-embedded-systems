/*
 * SISR TFLite Micro inference test for ESP32 - FSRCNN int8.
 *
 * Test shape:
...
#include "fsrcnn_int8_data.h"
#include "test_image.h"

constexpr int kMaxTensorArenaSize = 88 * 1024;
...
static uint8_t* allocate_tensor_arena(size_t* allocated_size) {
    const size_t candidate_sizes[] = {
        88 * 1024, 80 * 1024, 72 * 1024, 64 * 1024, 56 * 1024,
    };
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("\n=== SISR TFLite Micro Test (FSRCNN int8) ===");
    ...
    const tflite::Model* model = tflite::GetModel(fsrcnn_int8_tflite);
    ...
    resolver.AddPrelu();
    resolver.AddStridedSlice();   // actual onnx2tf op-set (verified from .tflite)
    resolver.AddAdd();
