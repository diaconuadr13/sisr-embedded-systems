/*
 * SISR TFLite Micro inference test for ESP32 - ESPCN_Light int8.
 *
 * Test shape:
...
#include "espcn_light_int8_data.h"
...
static uint8_t* allocate_tensor_arena(size_t* allocated_size) {
    const size_t candidate_sizes[] = {
        64 * 1024, 56 * 1024, 48 * 1024, 40 * 1024, 32 * 1024,
    };
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("\n=== SISR TFLite Micro Test (ESPCN_Light int8) ===");
    ...
    const tflite::Model* model = tflite::GetModel(espcn_light_int8_tflite);
    ...
    resolver.AddGather();   // onnx2tf pixel-shuffle export emits a GATHER
