...
## Vimeo-90K from Kaggle

This project uses the Kaggle mirror `wangsally/vimeo-90k-9` when the official Vimeo-90K download is too slow. Kaggle CLI credentials are required before download. Vimeo frames are treated as HR frames, and `LR_x2` or `LR_x4` frames are generated by bicubic downsampling. The prepared output is intended for PC-only inter-frame VSR training.
...
Then configure Kaggle credentials by creating an API token from Kaggle account settings and placing `kaggle.json` at `~/.kaggle/kaggle.json`, or by setting `KAGGLE_USERNAME` and `KAGGLE_KEY`.

Prepare x2 data:

```bash
python download_data.py --vimeo90k_kaggle --scale 2
```

Prepare a small smoke-test subset:
...
python download_data.py \
  ...
  --scale 2 \
  --limit_clips 100 \
  --max_frames_per_clip 7
```

Prepare x4 data:

```bash
python download_data.py --vimeo90k_kaggle --scale 4
```

Expected output layout:

```text
data/vimeo90k_vsr/train/clip_000000/HR/frame_000.png
data/vimeo90k_vsr/train/clip_000000/LR_x2/frame_000.png
data/vimeo90k_vsr/val/clip_000000/HR/frame_000.png
data/vimeo90k_vsr/val/clip_000000/LR_x2/frame_000.png
```

The prepared dataset is compatible with a VSR dataset class expecting:
...
```python
PairedVideoSRDataset(
    root_dir="data/vimeo90k_vsr",
    split="train",
    scale=2,
    num_frames=3,
)
